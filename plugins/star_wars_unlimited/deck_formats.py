...
def parse_json(deck_text: str, handle_card: Callable) -> None:
    ...
    def split_json_deck(deck_text: str):
        return loads(deck_text)

    def is_json_line(line) -> bool: # '{"id": "{Card Number with _ as separator}","count": {Quantity}}'
...
def parse_deck(deck_text: str, format: DeckFormat, handle_card: Callable) -> None:
    if format == DeckFormat.JSON:
        return parse_json(deck_text, handle_card)
