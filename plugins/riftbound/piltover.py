...
def fetch_card_art(card_number: str, quantity: int, front_img_dir: str):
    alternate_art_suffix_pattern = compile(r'^(\D{3}-\d{3})a$')
    signed_art_suffix = 's'

    card_front_image_query = f'https://piltoverarchive.com/_next/image?url=https%3A%2F%2Fcdn.piltoverarchive.com%2Fcards%2F{card_number}.webp&w=1920&q=75'
    success, api_response = request_api(card_front_image_query)
